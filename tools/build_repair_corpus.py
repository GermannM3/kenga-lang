...
def main():
    train_jsonl = 'minds/corpus_factory/split_v2/train.jsonl'
    test_jsonl = 'minds/corpus_factory/split_v2/test.jsonl'
    outdir = 'minds/repair_corpus'
    ...
    eval_recs = []
    for line in open(test_jsonl, encoding='utf-8'):
        line = line.strip()
        if not line:
            continue
        rec = json.loads(line)
    ...
    with open('minds/repair_corpus/test_mutants.jsonl', 'w', encoding='utf-8') as f:
        for r in eval_recs:
            f.write(json.dumps(r) + '\n')

    print(f'repair corpus: {n_docs} docs -> {outdir}/')
    print(f'modes: {modes}')
    print(f'eval mutants: {len(eval_recs)} -> test_mutants.jsonl')
    return 0
