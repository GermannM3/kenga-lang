#!/usr/bin/env bash
# tools/morning_report.sh — wait for M6 pipeline, then build M6_REPORT.md
set -u
cd "$(dirname "$0")/.." || exit 1
export PYTHONIOENCODING=utf-8

while ! grep -aq "\[m6\] done" pipeline_m6.log 2>/dev/null; do sleep 180; done
sleep 10
...
EOF
echo "[morning] report ready $(date +%H:%M:%S)"
