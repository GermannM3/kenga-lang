...
static char compound_op(const char *s, size_t j, size_t n) {
  if (j + 1 >= n || s[j + 1] != '=') return 0;
  if (s[j] == '+' || s[j] == '-' || s[j] == '*' || s[j] == '/') return s[j];
  return 0;
}
static void emit_compound_op(I64A *code, char op) {
  if (op == '+') i64a_push(code, OP_ADD);
  else if (op == '-') i64a_push(code, OP_SUB);
  else if (op == '*') i64a_push(code, OP_MUL);
  else i64a_push(code, OP_DIV);
}
/* Integer match: match <expr> { 0 => { ... } _ => { ... } } */
static size_t emit_match(const char *s, size_t i, size_t n, I64A *code, StrA *vnames,
  ...
  snprintf(mn, sizeof mn, "__m%d", g_tmpn++);
  mslot = slot_of(vnames, mn);
  emit2(code, OP_STORE, mslot);
  i64a_push(code, OP_POP);
  i = skip(s, i, n);
  if (i >= n || s[i] != '{') die("expected { after match");
  i++;
  for (;;) {
    int wild = 0;
    int64_t lit = 0;
    i = skip(s, i, n);
    if (i >= n) die("unclosed match");
    if (s[i] == '}') break;
    if (starts_kw(s, i, n, "_")) {
      wild = 1;
      i = skip(s, i + 1, n);
      ...
      if (i >= n || s[i] != '.' || (i + 1 < n && s[i + 1] == '.')) {
        free(id.name);
        die("expected . after enum name in match");
      }
      tid = find_struct_type(id.name);
      if (tid < 0) {
        free(id.name);
        die("unknown enum in match");
      }
      free(id.name);
      fld = parse_ident(s, i + 1, n);
      ...
      if (s[i] == '-') { neg = 1; i++; }
      ii = i;
      num = parse_number(s, &ii, n);
      if (num.is_float) die("match arm must be integer");
      lit = num.i;
      if (neg) lit = -lit;
      i = skip(s, ii, n);
    }
    if (!starts_with(s, i, n, "=>")) die("expected =>");
    i = skip(s, i + 2, n);
    if (!wild) {
      size_t jmpf_at, jmp_at;
      emit2(code, OP_LOAD, mslot);
      emit2(code, OP_CONST, lit);
  ...
  if (i >= n || s[i] != '}') die("expected } after match");
  i++;
  for (ji = 0; ji < jmps.len; ji++)
    code->data[jmps.data[ji] + 1] = (int64_t)code->len;
  i64a_free(&jmps);
  return i;
}
static size_t emit_stmt(const char *s, size_t i, size_t n, I64A *code, StrA *vnames,
  ...
  if (starts_kw(s, i, n, "for")) {
    /* for name in start..end [step k] { }  |  for name in list { } */
    i = skip(s, i + 3, n);
    ...
    if (starts_with(s, i, n, "..")) {
      ...
      i = skip(s, i, n);
      char stepn[32];
      snprintf(stepn, sizeof stepn, "__fs%d", g_tmpn++);
      int sslot = slot_of(vnames, stepn);
      if (starts_kw(s, i, n, "step")) {
        i = skip(s, i + 4, n);
        i = emit_cmp(s, i, n, code, vnames, fnames, faddrs, fargc);
      } else {
        emit2(code, OP_CONST, 1);
      }
      emit2(code, OP_STORE, sslot);
      i64a_push(code, OP_POP);
      size_t loop_start = code->len;
  ...
  if (starts_kw(s, i, n, "match")) {
    return emit_match(s, i, n, code, vnames, fnames, faddrs, fargc);
  }
  ...
  if (is_ident_start(s[i])) {
    ...
    if (j < n && s[j] == '[') {
      ...
      {
        char cop = compound_op(s, j, n);
        if (cop) {
          char ln[32], ix[32], vn[32];
          int lslot, islot, rslot;
          snprintf(ln, sizeof ln, "__cl%d", g_tmpn);
          snprintf(ix, sizeof ix, "__ci%d", g_tmpn);
          snprintf(vn, sizeof vn, "__cv%d", g_tmpn);
          g_tmpn++;
          lslot = slot_of(vnames, ln);
          islot = slot_of(vnames, ix);
          rslot = slot_of(vnames, vn);
          emit2(code, OP_STORE, islot);
          i64a_push(code, OP_POP);
          emit2(code, OP_STORE, lslot);
          i64a_push(code, OP_POP);
          ...
          emit2(code, OP_LOAD, islot);
          i64a_push(code, OP_GET);
          j = emit_cmp(s, j + 2, n, code, vnames, fnames, faddrs, fargc);
          emit_compound_op(code, cop);
          emit2(code, OP_STORE, rslot);
          i64a_push(code, OP_POP);
          emit2(code, OP_LOAD, lslot);
          emit2(code, OP_LOAD, islot);
          emit2(code, OP_LOAD, rslot);
          i64a_push(code, OP_SET);
          i64a_push(code, OP_POP);
          j = skip(s, j, n);
          if (j < n && s[j] == ';') j++;
      ...
      }
      {
        ...
        if (cop) {
          char vn[32];
          int fn, rslot;
          snprintf(vn, sizeof vn, "__cf%d", g_tmpn++);
          rslot = slot_of(vnames, vn);
          fn = intern_str(fld.name);
          free(fld.name);
          emit2(code, OP_LOAD, sl);
          emit2(code, OP_GET_FIELD, fn);
          j = emit_cmp(s, j + 2, n, code, vnames, fnames, faddrs, fargc);
          emit_compound_op(code, cop);
          emit2(code, OP_STORE, rslot);
          i64a_push(code, OP_POP);
