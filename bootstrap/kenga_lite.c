...
static void listh_free(ListHeap *h) {
  for (size_t i = 0; i < h->len; i++) vala_free(&h->data[i]);
  free(h->data);
  h->data = NULL;
  h->len = h->cap = 0;
}
...
static void structh_free(StructHeap *h) {
  for (size_t i = 0; i < h->len; i++) i64a_free(&h->data[i].fields);
  free(h->data);
  h->data = NULL;
  h->len = h->cap = 0;
}
...
/* break/continue patch stacks for while/for */
#define LOOP_MAX 16
...
/* Forward fn calls: faddrs is filled while compiling, so CALL addr is patched after. */
typedef struct {
  size_t addr_at;
  size_t fn_i;
} CallPatch;
...
static void call_patches_apply(I64A *code, const I64A *faddrs) {
  for (size_t i = 0; i < g_ncall_patches; i++) {
...
/* returns newly allocated unescaped string; advances *i past closing quote */
...
static size_t emit_cmp(const char *s, size_t i, size_t n, I64A *code, StrA *vnames,
  ...
  if (starts_with(s, i, n, "<=")) {
    ...
    i64a_push(code, OP_LE);
    return i;
  }
  if (starts_with(s, i, n, ">=")) {
    i = emit_sum(s, i + 2, n, code, vnames, fnames, faddrs, fargc);
    i64a_push(code, OP_GE);
    return i;
  }
  if (starts_with(s, i, n, "==")) {
    i = emit_sum(s, i + 2, n, code, vnames, fnames, faddrs, fargc);
    i64a_push(code, OP_EQ);
    return i;
  }
  if (starts_with(s, i, n, "!=")) {
    i = emit_sum(s, i + 2, n, code, vnames, fnames, faddrs, fargc);
    i64a_push(code, OP_NE);
    return i;
  }
  if (s[i] == '<') {
