...
Язык в git уже дальше: `more.kenga` гоняет Prophet/tape/ppm-wav/`learn`/`unroll`/living multimodal, `lower_kv` эмитит Memory и Tensor, `lower_c`/`bc_src_c` — `typeof`/`print`/`sleep_ms`/`now_ms`.  
