...
4. **`bc_src_c`**: parse `.kenga` → bytecode → generated C VM (lite + agent + **import** flatten + **read_file/write_file**); shared **`bootstrap/generated/bc_rt.inc.c`**. Одноразовый запуск: **`scripts\bc-run.cmd path\to\file.kenga`** → `bc_one_out.c`.  
