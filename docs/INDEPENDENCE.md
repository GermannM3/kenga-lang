...
4. **`bc_src_c`**: parse `.kenga` → bytecode → generated C VM (lite + agent + import + file I/O + **V-lists**). Своя сетка: **`examples/ml/kenga_net.kenga`**. Запуск одного файла: **`scripts\bc-run.cmd`**.  
