...
| ✅ | **`bc_src_c`** — parse → bytecode → C VM (lite suite + **struct** + **events/agent** + **import** + **fact_lite** + **runtime str**); shared **`bc_rt.inc.c`** runtime; **`scripts\bc-run.cmd`** — один `.kenga` → native |
