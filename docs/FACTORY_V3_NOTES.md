...
Лексеры `make_codec_tokenize` (`train_m3.py:118`) и `tokenize`
(`kenchat.py:79`) знают только старый синтаксис.

### P0 — молчаливые искажения

1. **`.` отсутствует в наборе пунктуации** и не попадает ни в одну ветку —
   точка молча выбрасывается. Следствие: `for i in 0..10` кодируется как
   `NUM NUM` без диапазона; `obj.field` и `Renderer.Framebuffer` разваливаются.
   Нужен терминал `..` (+ опционально `.`).
2. **`&`, `|`, `^`, `~` сидят в TWO_CHAR, но их нет в vocab** — fallback
   `VOCAB_MAP.get(two, VOCAB_MAP['ID'])` отображает битовые операторы в `ID`.
   Чинить: добавить терминалы или убрать из TWO_CHAR.
3. **`=>` (FatArrow)** не обрабатывается: спец-ветка есть только для `->`
   (`train_m3.py:60`) → `=>` распадается на `=` `>`, неотличимо от сравнения.
   Обязательно для match-arms.
4. **`::`** распадается на два `:` — неоднозначно с аннотацией типа.
   Желателен терминал.
